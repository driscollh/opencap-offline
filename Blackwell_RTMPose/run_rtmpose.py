...
def process_video_rtmpose(video_path, output_dir, video_out_dir, device='cuda:0', model_complexity='m'):
    dep_dir = Path("./dependencies/rtmpose")
    
    # Specifically target the complexity (m or l) passed from the GUI
    pose_config = str(list(dep_dir.glob(f"rtmpose-{model_complexity}*.py"))[0])
    pose_ckpt = str(list(dep_dir.glob(f"rtmpose-{model_complexity}*.pth"))[0])
    
    det_config = str(list(dep_dir.glob("rtmdet_m*.py"))[0])
    det_ckpt = str(list(dep_dir.glob("rtmdet_m*.pth"))[0])
