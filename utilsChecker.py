...
def loadPklVideo(pklPath, videoFullPath, imageBasedTracker=False, poseDetector='OpenPose',
    ...
    print("pkl file successfully opened")
...
def popNeutralPoseImages(cameraDirectories, camerasToUse, tSingleImage,
    ...
    for iCam,cam in enumerate(cameras2Use):
        ...
        # --- FIX START: Dynamic Video Extension Check ---
        input_media_dir = os.path.join(cameraDirectories_selectedCams[cam], 
                                 'InputMedia', 'neutral')
        
        # Default to .mov if not found, but check others first
        videoPath = os.path.join(input_media_dir, '{}.mov'.format(trial_id))
        
        for ext in ['.mp4', '.mov', '.avi']:
            potential_path = os.path.join(input_media_dir, '{}{}'.format(trial_id, ext))
            if os.path.exists(potential_path):
                videoPath = potential_path
                break
        # --- FIX END ---
        
        if not os.path.exists(videoPath):
            print(f"WARNING: popNeutralPoseImages could not find video at: {videoPath}")
