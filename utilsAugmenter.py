...
# --- CACHE: Global dictionary to store loaded models ---
_MODEL_CACHE = {}
# -----------------------------------------------------
...
def augmentTRC(pathInputTRCFile, subject_mass, subject_height,
    ...
    for idx_augm, augmenterModelType in enumerate(augmenterModelType_all):
        ...
        # %% Load model (Cached)
        # --- FIX: Check cache first to avoid retracing warnings ---
        if augmenterModelDir in _MODEL_CACHE:
            model = _MODEL_CACHE[augmenterModelDir]
        else:
            ...
            # Save to cache
            _MODEL_CACHE[augmenterModelDir] = model
        
        # Predict (verbose=0 suppresses the progress bar)
        if inputs.ndim == 2: inputs = np.expand_dims(inputs, axis=0)
        if inputs.ndim == 2: inputs = np.expand_dims(inputs, axis=0)
        if inputs.ndim == 2: inputs = np.expand_dims(inputs, axis=0)
        if inputs.ndim == 2: inputs = np.expand_dims(inputs, axis=0)
        if inputs.ndim == 2: inputs = np.expand_dims(inputs, axis=0)
        ...
        if outputs.ndim == 3: outputs = np.squeeze(outputs, axis=0)
        if outputs.ndim == 3: outputs = np.squeeze(outputs, axis=0)
        if outputs.ndim == 3: outputs = np.squeeze(outputs, axis=0)
        if outputs.ndim == 3: outputs = np.squeeze(outputs, axis=0)
        if outputs.ndim == 3: outputs = np.squeeze(outputs, axis=0)
