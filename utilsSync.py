...
def synchronizeVideos(CameraDirectories, trialRelativePath, pathPoseDetector,
    ...
    print("\n  --- Starting Video Synchronization ---")
    ...
    for camName in CameraDirectories_selectedCams:
        ...
        # 5. FIX: Check for _rotated.avi, fallback to original if missing
        rotated_vid_path = videoFullPath.replace('.mov', '_rotated.avi')
        if os.path.exists(rotated_vid_path):
            video_to_load = rotated_vid_path
        else:
            video_to_load = videoFullPath 
            
        thisVideo = cv2.VideoCapture(video_to_load)
        frameRate = np.round(thisVideo.get(cv2.CAP_PROP_FPS))
        
        if key2D.shape[1] == 0 and confidence.shape[1] == 0:
            print(f"    [WARNING] No valid data found for {camName}. Excluding from sync.")
            camsToExclude.append(camName)
        else:
            pointList.append(key2D)
            confList.append(confidence)

    print("  > All camera data loaded successfully.")
...
def synchronizeVideoKeypoints(keypointList, confidenceList,
    ...
    # Around line 234 in utilsSync.py
    if len(keypointList) == 0:
        raise ValueError(f"No cameras remained after filtering for confidence. "
                         f"Check if OpenPose actually produced data for trial: {trialName}")
